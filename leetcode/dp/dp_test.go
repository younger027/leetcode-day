package dp

import (
	"testing"
)

func TestFfib(t *testing.T) {
	//t.Log(fib(4))
	//
	//t.Log(climbStairs(3))
	//
	//data := [][]int{
	//	{0, 0, 0},
	//	{0, 1, 0},
	//	{0, 0, 0},
	//}
	//
	//t.Log(uniquePathsWithObstacles(data))

	//t.Log(integerBreak(10))

	//t.Log(canPartition([]int{1, 5, 11, 5}))
	//t.Log(findRepeatNumber([]int{2, 3, 1, 0, 2, 5, 3}))

	//t.Log(canPartition([]int{1, 5, 5, 11}))
	//t.Log(lastStoneWeightII([]int{2, 7, 4, 1, 8, 1}))

	//t.Log(combinationSum4([]int{1, 2, 3}, 4))
	//t.Log(longestCommonSubsequenceDp("123", "avc"))
	//t.Log(minDistance("a", "b"))

	//t.Log(packageSplit(20, []int{1, 2, 19, 18}))
	//t.Log(countSubstrings("abc"))

	//t.Log(longestPalindromeSubSeq("cbbd"))
	//t.Log(longestPalindromeSubseq("cbbd"))
	//t.Log(dailyTemperatures([]int{73, 74, 75, 71, 69, 72, 76, 73}))
	//t.Log(dailyTemperaturesOP([]int{73, 74, 75, 71, 69, 72, 76, 73}))

	t.Log(nextGreaterElementMyself([]int{137, 59, 92, 122, 236},
		[]int{137, 59, 92, 122, 236}))
}
